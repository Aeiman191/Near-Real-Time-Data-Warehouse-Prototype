...
//__________________________________DIMENSION TABLES_________________________________________________//
        ...
        String day = f.format(entry.getValue().T_DATE);
        @SuppressWarnings("deprecation")
		int month = (entry.getValue().T_DATE).getMonth()+1;
		@SuppressWarnings("deprecation")
		int year = (entry.getValue().T_DATE).getYear();
        ...
        if (!ch_t.isBeforeFirst()) {
          String insert_time =
            "INSERT INTO TIME (TIME_ID,T_DATE,T_DAY,T_MONTH,T_YEAR) VALUES (\"" +
            entry.getValue().TIME_ID +
            "\",\"" +
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // ************************* Creating partitions in Transactions Table *************************************//
            ...
            String sql2 = "ALTER TABLE CUSTOMERS PARTITION BY KEY() PARTITIONS 5";
            stmt.executeUpdate(sql2);
            String sql3 = "ALTER TABLE PRODUCTS PARTITION BY KEY() PARTITIONS 5";
...
//          System.out.format("COUNT: %s\n ", key_count);
          ...
          for (int j = 0; j < partitions_m.length; j++) {
            // System.out.format("Master Data Partition : %s\n ", j+1);

            String sql5 =
              "SELECT * FROM CUSTOMERS partition(" + partitions_m[j] + ")";
            ...
            String sql6 =
              "SELECT * FROM PRODUCTS partition(" + partitions_m[j] + ")";
